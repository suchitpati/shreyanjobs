  ...
  setup() {
    ...
    const forgotPassword = async () => {
      try {
        ...
        await axios
          .post(`${apiUrl}/seeker-send-forgot-email-otp`, {
            email: email.value,
          })
          .then((response) => {
            ...
            if (response.data.error == 100) {
              ...
              steps.value = 2;
              seeker_email.value = response.data.email;
    ...
    const validateOtp = async () => {
      try {
        ...
        await axios
          .post(`${apiUrl}/seeker-check-forgot-otp`, {
            otp: final_otp.value,
            seeker_email: seeker_email.value,
          })
          .then((response) => {
            console.log(response);
            isLoading.value = false;
    ...
    const updatePassword = async () => {
      ...
      if (password.value != confirmPassword.value) {
        confirmPasswordError.value = "Enter same password";
        return false;
      } else {
        confirmPasswordError.value = "";
      }
      ...
      formData.append("seeker_email", seeker_email.value);

      try {
        await axios
          .post(`${apiUrl}/seeker-update-forgot-password`, formData)
          .then((response) => {
            console.log(response);
            isLoading.value = false;
            if (response.data.error == 100) {
